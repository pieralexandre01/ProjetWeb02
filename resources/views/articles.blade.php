        ...
        <div class="page_title">
            ...
            <div class="category_selector container pt-5 d-flex flex-column gap-4 gap-xl-4 gap-xxl-5">
                @foreach ($categories as $category)
                    <span>
                        <a href="#{{ $category->name_reference }}"
                            class="category_button d-inline-block">{{ $category->name }}</a>
                    </span>
                @endforeach
            </div>
        </div>

        <div class=""><img src="{{ asset('/../media/images/articles.webp') }}" class="w-100"
                alt="Digital image of a robotic hand and a human hand sharing an electric current"></div>
                    ...
                    @foreach ($articles as $article)
                        @if ($article->category->name == $category->name)
                            <div class="article_preview border_box mobile_frame d-flex flex-column py-5">
                                ...
                                <div
                                    class="d-flex flex-column flex-md-row justify-content-md-between align-items-md-end mt-4">
                                    <p class="mb-0">{{ $article->summary }}</p>

                                    <div><a href="{{ route('article', $article->id) }}"
                                            class="general_button d-block d-md-inline-block text-center mt-5 mt-md-0">read
                                            more</a></div>
                                </div>
