                ...
                @foreach ($activities as $activity)
                    <div class="accordion-item">
                        <div class="accordion-header">
                            <button
                                class="accordion-button @if ($activity->id != 1) collapsed @endif px-4 px-md-5"
                                type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $activity->id }}"
                                aria-expanded="false" aria-controls="collapse{{ $activity->id }}">
                                {{ $activity->title }}
                            </button>
                        </div>

                        <div id="collapse{{ $activity->id }}"
                            class="accordion-collapse collapse @if ($activity->id == 1) show @endif"
                            data-bs-parent="#activity_accordion">
                            ...
                            <div class="accordion-body px-sm-4 px-md-5">
                                ...
                                <div class="d-flex flex-column flex-md-row-reverse align-items-center">
                                    <div class="d-flex flex-column align-items-center">
                                        <img src="{{ asset('/../' . $activity->image) }}" class="activity_image"
                                            alt="">
                                        <div class="activity_date align-self-end my-3 mb-md-0 d-none d-md-block">
                                            {{ $activity->formatted_date }}</div>
                                    </div>

                                    <div class="activity_date align-self-end my-3 d-md-none">
                                        {{ $activity->formatted_date }}</div>
            ...
            <section id="activity_list" class="d-flex flex-column me-5 justify-content-between">
                <div class="d-flex flex-column justify-content-between">
                    @foreach ($activities as $key => $activity)
                        <button class="activity_button px-3 text-start" type="button"
                            @@click="activity = {{ $key }}"
                            v-show="activity_list == {{ floor($key / 9) + 1 }}">
                            {{ $activity->title }}
                        </button>
                    @endforeach
                </div>
                ...
                <div class="activity_list_pagination text-center d-flex justify-content-center align-items-center">
                    <button v-show="{{ count($activities) >= 10 }}" class="activity_list_button"
                        @@click="activity_list = 1">1</button>
                    <button v-show="{{ count($activities) >= 10 }}" class="activity_list_button"
                        @@click="activity_list = 2">2</button>
                    <button v-show="{{ count($activities) >= 19 }}" class="activity_list_button"
                        @@click="activity_list = 3">3</button>
                </div>
            </section>
                ...
                @foreach ($activities as $key => $activity)
                    <div class="activity border_box p-5" v-show="{{ $key }} == activity">
                        ...
                        <div class="d-flex flex-column flex-md-row-reverse align-items-center mt-4">
                            ...
                            <div class="activity_date align-self-end my-3 d-md-none">{{ $activity->formatted_date }}
                            </div>
